...
def register(reg: User):
    ...
    response = requests.post(SUPABASE_URL + SUPABASE_REGISTER, headers=headers(), json=data)
...
def login(log: LoginIn):
    ...
    try:
        ok = pwd_context.verify(log.password, stored_hash)
    ...
    resp_pub = requests.get(url, headers=headers(), params=params_pub)
    if not resp_pub.ok:
